...
|목적 함수| 모델 파라미터 | 예시 |
|---|---|---|
|**비커널 알고리즘**   | w  | RR(Ridge Regression), LASSO, LR(Logistic Regression), SVM(Support Vector Machine)        |
| **커널 알고리즘**    | w 또는 α | KLR(Kernel Logistic Regression), KSVM(Kernel Support Vector) |
...
그럴 경우, 가장 근사한 λ를 계산하기 위하여  **최소자승법**(Least-Squares solution)을 사용해 추정치를 계산합니다.
$$
\begin{align}
...
그리고 실제 MLaaS 플랫폼에서의 하이퍼파라미터 추정 성능까지 분석합니다.

마지막으로 반올림 기반 방어법의 성능과 한계도 밝힙니다.
...
위 그래프를 보면 소수점 자릿수가 낮을 때, L2 정규화를 사용하는 목적 함수가 L1 정규화를 사용하는 목적 함수보다 평가지표들이 높습니다.
즉, **L2 정규화**를 사용할 때 하이퍼파라미터 도용 공격에 대한 방어 성능이 더 좋다는 의미로 해석할 수 있습니다.
L2 정규화는 L1 정규화와 달리 미분값이 연속적이어서, 반올림으로 인한 변화가 그대로 반영됩니다. 그로 인해 하이퍼파라미터 상대추정오차가 더 효과적으로 증가하고, 도용 공격 성공 가능성을 더 잘 줄일 수 있습니다.

**MLaaS 실험결과**

아래 그래프는 MLaaS의 M3 방법에 대해 반올림 여부를 비교합니다.
이때 반올림 반올림 방어를 했을 때에 평가지표가 더 높은 모습을 보여주며, 반올림 방어가 MLaaS에서도 효과적임을 보여줍니다.
